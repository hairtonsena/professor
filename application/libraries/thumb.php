...
class browser extends image {
    ...
    private function output($path) {
        switch (image::getMimeType()) {
            case "image/jpeg": imagejpeg($this->resource, $path, image::getJpegQuality());
                break;
            case "image/png": imagepng($this->resource, $path, image::getPngQuality());
                break;
            case "image/gif": imagegif($this->resource, $path, image::getGifQuality());
                break;
            default: imagejpeg($this->resource, $path, image::getJpegQuality());
                break;
        }
    }
...
class thumb extends browser {
    ...
    private function imageCreate() {
        switch (image::getMimeType()) {
            case "image/jpeg": $this->stream = imagecreatefromjpeg(image::getImage());
                break;
            case "image/png": $this->stream = imagecreatefrompng(image::getImage());
                break;
            case "image/gif": $this->stream = imagecreatefromgif(image::getImage());
                break;
